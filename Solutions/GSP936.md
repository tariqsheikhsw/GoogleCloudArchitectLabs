### GSP936 :  Modularizing LookML Code with Extends 


## Login to Looker
### Toggle Development mode button > Develop > qwiklabs-ecommerce 
> Click on three dots next to views > Create View > Name ```location```
## Replace this code
```cmd
view: location {
  extension: required
...
```
>Save Changes > Validate LookML
### Open users.view > Replace the code 
```cmd
...
```
>Save Changes > Validate LookML
### Open event.view > Replace the code 
```cmd
include: location.view
view: events {
  extends: [location]
...
```
>Save Changes > Validate LookML > Commit Changes & Push 

>Message `CloudHustlers` > commit > Deploy to Production
### Open models > training_ecommerce.model > Replace the code 
```cmd
