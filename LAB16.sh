gcloud config set project qwiklabs-gcp-02-967f79ef3540
...
gcloud compute target-vpn-gateways create on-prem-gw1 --network on-prem --region us-east1
...
gcloud compute addresses create on-prem-gw1 --region us-east1
...
gcloud compute forwarding-rules create on-prem-fr-esp --ip-protocol ESP \
    --address $on_prem_gw_ip --target-vpn-gateway on-prem-gw1 --region us-east1
...
gcloud compute vpn-tunnels create cloud-tunnel1 --peer-address $on_prem_gw_ip \
    ...
    --remote-traffic-selector 0.0.0.0/0 --shared-secret=sharedsecret --region europe-west1

//gcloud compute vpn-tunnels delete on-prem-tunnel1 --region us-east1
//gcloud compute vpn-tunnels delete cloud-tunnel1 --region europe-west1
...
gcloud compute instances create "cloud-loadtest" --zone europe-west1-d \
...
gcloud compute instances create "on-prem-loadtest" --zone us-east1-d \
